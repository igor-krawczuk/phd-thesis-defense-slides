...
\begin{frame}[label=predesiderata,t]
  \frametitle{Why no learned graph generation for topology synthesis?}
  \begin{itemize}
    \item Topology Data is scarce\ffootnote{\cite{chowdhuryOpenABCDLargeScaleDataset2021e,chaiCircuitNetOpenSourceDataset2023b, panEDALearnComprehensiveRTLtoSignoff2023,weiHLSDatasetOpenSourceDataset2023}}...
    \item ...and even if we want to use RL,we probably need data for pretraining\ffootnote{\cite{gulcehreKnowledgeMattersImportance2016,paineMakingEfficientUse2019a,schwarzerPretrainingRepresentationsDataEfficient2021,agarwalReincarnatingReinforcementLearning2022a}}\ffootnote{\cite{vlassisBayesianReinforcementLearning2012a,buesingLearningQueryingFast2018,sarmadRLGANNetReinforcementLearning2019b}}
    \item but even with data, we require suitable generative models to parametrize!
  \end{itemize}
\end{frame}

\begin{frame}[label=desiderata,t]
  \frametitle{Desiderata for graph generators applied to circuit topologies}
  {
  \small
    \vspace{-0.9cm}
  \begin{desideratum}
