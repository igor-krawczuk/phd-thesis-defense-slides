...
\begin{frame}[label=ggg,t,fragile]
  ...
  \only<1-5>{
        ...
        \only<4-5>{
          \item MPNN has $\mathcal{O}\left(n\right)$ but expressivity constraints\footnotemark[3], $\mathcal{O}\left(n^3\right)$ for more expressive models \ffootnote{\cite{maronProvablyPowerfulGraph2019b,vignacBuildingPowerfulEquivariant2020,balcilarBreakingLimitsMessage2021b}}% either in model for for pre-computation
  }
  \only<5>{
...
\begin{frame}[label=ggg]
  ...
  \begin{itemize}
    ...
    \item Score-Matching + Langevin Dynamics(LD)\ffootnote{\cite{niuPermutationInvariantGraph2020b}}  $\centernot\implies$ $\SnE$,but \emph{extremely} slow\&expensive sampling 
  \end{itemize}
\end{frame}
...
\begin{frame}[label=ggg,t]
  \frametitle{Collision problem \only<3->{resolved}}
  \only<1>{
  \includegraphics[width=\textwidth,height=\textheight,keepaspectratio]{./images/rsg.pdf}
}
  \only<2>{
...
\only<4>{
  \includegraphics[height=0.7\textheight,keepaspectratio]{./images/CollisionDecentiles-Att.pdf}\\
  {\small
...
\begin{frame}[label=ggg,t]
  ...
  \vspace{-0.5cm}
  \centering
    \includegraphics[width=\textwidth,height=0.45\textheight,keepaspectratio]{./images/ggg_results_2024-02-25_00-52.png}
  \visible<2>{
    \includegraphics[width=\textwidth,height=0.45\textheight,keepaspectratio]{./images/drawing.pdf}
  }
\end{frame}
\begin{frame}[label=ggg,t]
  \frametitle{Results (latency)}
  \includegraphics[width=\textwidth,height=0.85\textheight,keepaspectratio]{./images/scale_jointFalse.pdf}
\end{frame}
