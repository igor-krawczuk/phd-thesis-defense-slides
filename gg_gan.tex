...
\only<3->{
  \raggedright
  Data is sparse\ffootnote{\cite{chowdhuryOpenABCDLargeScaleDataset2021e,chaiCircuitNetOpenSourceDataset2023b, panEDALearnComprehensiveRTLtoSignoff2023,weiHLSDatasetOpenSourceDataset2023}} \emph{and} even with data, we require models that can...
\begin{itemize}
...
\begin{frame}[label=working]
  ...
  4 main variants
  \begin{itemize}
    \item Autoregressive likelihood(AR)\ffootnote{ \cite{youGraphRNNGeneratingRealistic2018a,liaoEfficientGraphGeneration2019e,daiScalableDeepGenerative2020e}} $\centernot\implies$ order dependent\ffootnote{\cite{chenOrderMattersProbabilistic2021c}},high latency
...
\begin{frame}[t]
  ...
  \visible<2->{
    \begin{column}{0.5\textwidth}
      \begin{outcomes}
      \strut
        First fully PE implicit GGM,scaling to $n\approx  100$\textuparrow,low latency \textuparrow,training stability \textdownarrow\vspace{6mm}
      \strut
      \end{outcomes} 
    \end{column}
}
