...
\begin{frame}[label=ggg,t,fragile]
  ...
  \only<1-5>{
        ...
        \only<4-5>{
          \item MPPN has $\mathcal{O}\left(n\right)$ but expressivity constraints\footnotemark[5], $\mathcal{O}\left(n^3\right)$ for more expressive models \ffootnote{\cite{maronProvablyPowerfulGraph2019b,vignacBuildingPowerfulEquivariant2020,balcilarBreakingLimitsMessage2021b}}% either in model for for pre-computation
  }
  \only<5>{
...
\begin{frame}[t,label=gggfin]
  ...
  \visible<1->{
    \begin{column}{0.5\textwidth}
      \begin{outcomes}
      \strut
        First fully $\SnE$ implicit model,scaling to $n\approx  100$\textuparrow,low latency \textuparrow\\training stability \textdownarrow\vspace{6mm}
      \strut
      \end{outcomes} 
    \end{column}
}
